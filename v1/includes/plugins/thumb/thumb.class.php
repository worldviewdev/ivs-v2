...
class midas_thumb
{
	static function get_thumb_vars($imgPath, $newWidth, $newHeight, $ratio_type)
	{
	...
	static function make_thumb_imagemagick($imgPath, $destPath, $newWidth, $newHeight, $ratio_type = 'width', $quality = 80, $verbose = false)
	{
		$quality = 80;
	...
	static function make_thumb_gd($imgPath, $destPath, $newWidth, $newHeight, $ratio_type = 'width', $quality = 70, $verbose = false)
	{
		// options for ratio type = width|height|width_height|distort|crop
		$arr_thumb_vars = midas_thumb::get_thumb_vars($imgPath, $newWidth, $newHeight, $ratio_type);
		...
		// Tambahan untuk transparansi PNG & GIF
		...
		imagecopyresampled($resizedImage, $originalImage, 0, 0, (int)$srcX, (int)$srcY, (int)$newWidth, (int)$newHeight, (int)$srcWidth, (int)$srcHeight);
		...
		switch ($fileExt) {
			...
			case 'jpg':
				...
				break;
			case 'png':
				// Konversi quality 0-100 ke 0-9
				$pngQuality = 9 - min(9, floor($quality / 10));
				imagepng($resizedImage, $destPath, $pngQuality);
				break;
		}
		// return true if successfull
		return true;
	}
	static function make_thumb($imgPath, $destPath, $newWidth, $newHeight, $ratio_type = 'width', $method = 'imagemagick', $quality = 70, $verbose = false)
	{
	...
	static function show_thumb($file_org, $width, $height, $ratio_type = 'width', $method = '', $mode = 'php')
	{
		$width = intval($width);
		...
		$cache_file = $width . "x" . $height . '__' . $ratio_type . '__' . (file_exists($file_fs_path) ? filemtime($file_fs_path) : time()) . '__' . $file_name . "." . $file_ext;
