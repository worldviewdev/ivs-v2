...
class midas_thumb {
	...
	function show_thumb($file_org, $width, $height, $ratio_type = 'width', $method= 'imagemagick', $mode='php')
	{
		// mode php|embedded
		...
		if(!is_file(SITE_FS_PATH."/".THUMB_CACHE_DIR."/".$cache_file)) {
			if($mode=='php') {
				return get_absolute_dir(__FILE__)."/show_thumb.php?path=".rawurlencode(fs_to_absolute($file_fs_path))."&cache_file=".rawurlencode($cache_file)."&width=".$width."&height=".$height."&ratio_type=".$ratio_type."&method=".$method;
			} else {
				midas_thumb::make_thumb($file_fs_path, SITE_FS_PATH."/".THUMB_CACHE_DIR."/".$cache_file, $width, $height, $ratio_type, $method);
				return SITE_WS_PATH."/".THUMB_CACHE_DIR."/".rawurlencode($cache_file);
			}
		} else {
			return SITE_WS_PATH."/".THUMB_CACHE_DIR."/".rawurlencode($cache_file);
		}
