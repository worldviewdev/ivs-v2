...
$root_path = $_SERVER['DOCUMENT_ROOT'].'/ivsportal/uploaded_files/media';
// Root url for links in file manager.Relative to $http_host. Variants: '', 'path/to/subfolder'
// Will not working if $root_path will be outside of server document root
$root_url = 'ivsportal/uploaded_files/media';
...
$allowed_upload_extensions = 'jpg,jpeg,png,gif,bmp,webp,svg,tiff,tif,ico';
...
if (defined('FM_EMBED')) {
    $use_auth = false;
    $sticky_navbar = false;
    $global_readonly = false; // Allow full access in embed mode
} else {
    @set_time_limit(600);
...
// For embedded mode, use SCRIPT_NAME instead of PHP_SELF to get correct URL
if (defined('FM_EMBED') && FM_EMBED) {
...
if (!empty($_FILES) && !FM_READONLY) {
    ...
    if (is_writable($targetPath)) {
        // Pastikan fullPathInput berisi nama file, jika tidak gunakan filename dari upload
        if (is_dir($path . '/' . $fullPathInput) || empty($fullPathInput) || $fullPathInput === FM_PATH) {
...
if (isset($_GET['upload']) && !FM_READONLY) {
    ...
    <div class="path">
        <div class="card mb-2 fm-upload-wrapper" data-bs-theme="<?php echo FM_THEME; ?>">
            ...
            <div class="card-body">
                <p class="card-text">
                    <a href="<?php echo FM_SELF_URL ?>?p=<?php echo FM_PATH ?>" class="float-right"><i class="fa fa-chevron-circle-left go-back"></i> <?php echo lng('Back') ?></a>
    ...
    <script>
        // Dropzone untuk standalone upload page (dinonaktifkan karena kita pakai modal)
        // Dropzone.options.fileUploader sudah dihandle di modal
        // Disable auto-discover untuk mencegah konflik
        ...
        <div class="card" data-bs-theme="<?php echo FM_THEME; ?>">
            ...
            <div class="card-body">
                <form action="" method="post">
                    ...
                    <p>
                        <b><a href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>" class="btn btn-outline-danger"><i class="fa fa-times-circle"></i> <?php echo lng('Cancel') ?></a></b>&nbsp;
                        <input type="hidden" name="token" value="<?php echo $_SESSION['token']; ?>">
...
?>
    <div class="path">
        ...
        <p>
            <b><a href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1"><i class="fa fa-check-circle"></i> Copy</a></b> &nbsp;
            <b><a href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1&amp;move=1"><i class="fa fa-check-circle"></i> Move</a></b> &nbsp;
            <b><a href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>" class="text-danger"><i class="fa fa-times-circle"></i> Cancel</a></b>
        </p>
        ...
        <ul class="folders break-word">
            ...
            if ($parent !== false) {
            ?>
                <li><a href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode($parent) ?>&amp;copy=<?php echo urlencode($copy) ?>"><i class="fa fa-chevron-circle-left"></i> ..</a></li>
            <?php
            }
            foreach ($folders as $f) {
            ?>
                <li>
                    <a href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>&amp;copy=<?php echo urlencode($copy) ?>"><i class="fa fa-folder-o"></i> <?php echo fm_convert_win($f) ?></a>
                </li>
            <?php
            }
            ?>
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="btn-group btn-group-sm flex-wrap" role="group">
                <form method="post" class="d-inline mb-0 btn btn-outline-primary" action="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($file) ?>">
                    <input type="hidden" name="token" value="<?php echo $_SESSION['token']; ?>">
                ...
                <?php if (!FM_READONLY): ?>
                    <a class="fw-bold btn btn-outline-primary" title="<?php echo lng('Delete') ?>" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($file) ?>" onclick="confirmDailog(event, 1209, '<?php echo lng('Delete') . ' ' . lng('File'); ?>','<?php echo urlencode($file); ?>', this.href);"> <i class="fa fa-trash"></i> Delete</a>
                <?php endif; ?>
                ...
                <?php } ?>
                <a class="fw-bold btn btn-outline-primary" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>"><i class="fa fa-chevron-circle-left go-back"></i> <?php echo lng('Back') ?></a>
            </div>
            <div class="row mt-3">
                <?php
                if ($is_onlineViewer) {
    ...
    <div class="path">
        <div class="row">
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-end pt-1">
                <div class="btn-group">
                    <a title=" <?php echo lng('Back') ?>" class="btn btn-sm btn-outline-primary" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;view=<?php echo urlencode($file) ?>"><i class="fa fa-reply-all"></i> <?php echo lng('Back') ?></a>
                    <a title="<?php echo lng('BackUp') ?>" class="btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="backup('<?php echo urlencode(trim(FM_PATH)) ?>','<?php echo urlencode($file) ?>')"><i class="fa fa-database"></i> <?php echo lng('BackUp') ?></a>
                    <?php if ($is_text) { ?>
                        <?php if ($isNormalEditor) { ?>
                            <a title="Advanced" class="btn btn-sm btn-outline-primary" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&amp;env=ace"><i class="fa fa-pencil-square-o"></i> <?php echo lng('AdvancedEditor') ?></a>
                        ...
                        <?php } else { ?>
                            <a title="Plain Editor" class="btn btn-sm btn-outline-primary" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>"><i class="fa fa-text-height"></i> <?php echo lng('NormalEditor') ?></a>
        ...
        <div class="card mb-2" data-bs-theme="<?php echo FM_THEME; ?>">
            ...
            <div class="card-body">
                ...
                <form action="" method="post">
                    ...
                    <p>
                        <input type="hidden" name="token" value="<?php echo $_SESSION['token']; ?>">
                        <b><a href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>" class="btn btn-outline-primary"><i class="fa fa-times-circle"></i> <?php echo lng('Cancel') ?></a></b>&nbsp;
        ...
        <table class="table table-bordered table-hover table-sm" id="main-table" data-bs-theme="<?php echo FM_THEME; ?>">
            ...
            ?>
                <tr><?php if (!FM_READONLY): ?>
                    ...
                    <td class="border-0" data-sort><a href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode($parent) ?>"><i class="fa fa-chevron-circle-left go-back"></i> ..</a></td>
            ...
            ?>
                <tr>
                    ...
                    <td data-sort=<?php echo fm_convert_win(fm_enc($f)) ?>>
                        <div class="filename">
                            <a href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="<?php echo $img ?>"></i> <?php echo fm_convert_win(fm_enc($f)) ?></a>
                    ...
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td>
                            <?php if (!FM_READONLY): ?><a title="Change Permissions" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a><?php else: ?><?php echo $perms ?><?php endif; ?>
                        </td>
                        <td>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete') ?>" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, '1028','<?php echo lng('Delete') . ' ' . lng('Folder'); ?>','<?php echo urlencode($f) ?>', this.href);"> <i class="fa fa-trash-o" aria-hidden="true"></i></a>
                            <a title="<?php echo lng('Rename') ?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                            <a title="<?php echo lng('CopyTo') ?>..." href="<?php echo FM_SELF_URL ?>?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o" aria-hidden="true"></i></a>
                        <?php endif; ?>
            ...
            foreach ($files as $f) {
                ...
                $filesize = fm_get_filesize($filesize_raw);
                $filelink = FM_SELF_URL . '?p=' . urlencode(FM_PATH) . '&amp;view=' . urlencode($f);
            ...
            ?>
                <tr>
                    ...
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td><?php if (!FM_READONLY): ?><a title="<?php echo 'Change Permissions' ?>" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a><?php else: ?><?php echo $perms ?><?php endif; ?>
                        </td>
                        <td><?php echo fm_enc($owner['name'] . ':' . $group['name']) ?></td>
                    <?php endif; ?>
                    <td class="inline-actions">
                        <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete') ?>" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, 1209, '<?php echo lng('Delete') . ' ' . lng('File'); ?>','<?php echo urlencode($f); ?>', this.href);"> <i class="fa fa-trash-o"></i></a>
                            <a title="<?php echo lng('Rename') ?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-pencil-square-o"></i></a>
                            <a title="<?php echo lng('CopyTo') ?>..."
                                href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o"></i></a>
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink') ?>" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="fa fa-link"></i></a>
                        <a title="<?php echo lng('Download') ?>" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, 1211, '<?php echo lng('Download'); ?>','<?php echo urlencode($f); ?>', this.href);"><i class="fa fa-download"></i></a>
                    </td>
                </tr>
            <?php
                flush();
...
?>
    <nav class="navbar navbar-expand-lg mb-4 main-nav <?php echo $isStickyNavBar ?> bg-body-tertiary" data-bs-theme="<?php echo FM_THEME; ?>">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <?php
            $path = fm_clean_path($path);
            $root_url = "<a href='".FM_SELF_URL."?p='><i class='fa fa-home' aria-hidden='true' title='" . FM_ROOT_PATH . "'></i></a>";
            ...
            if ($path != '') {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    $array[] = "<a href='".FM_SELF_URL."?p={$parent_enc}'>" . fm_enc(fm_convert_win($exploded[$i])) . "</a>";
                }
            ...
            <div class="col-xs-6 col-sm-7">
                <ul class="navbar-nav justify-content-end" data-bs-theme="<?php echo FM_THEME; ?>">
                    ...
                    <?php if (!FM_READONLY): ?>
                        <li class="nav-item">
                            <a title="<?php echo lng('Upload') ?>" class="nav-link" href="#uploadModal" data-bs-toggle="modal" data-bs-target="#uploadModal"><i class="fa fa-cloud-upload" aria-hidden="true"></i> <?php echo lng('Upload') ?></a>
                        </li>
                        <li class="nav-item">
                    ...
                    <?php if (FM_USE_AUTH): ?>
                        <li class="nav-item avatar dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end text-small shadow" aria-labelledby="navbarDropdownMenuLink-5" data-bs-theme="<?php echo FM_THEME; ?>">
                                <?php if (!FM_READONLY): ?>
                                    <a title="<?php echo lng('Settings') ?>" class="dropdown-item nav-link" href="#settingsModal" data-bs-toggle="modal" data-bs-target="#settingsModal"><i class="fa fa-cog" aria-hidden="true"></i> <?php echo lng('Settings') ?></a>
                                <?php endif ?>
                                <a title="<?php echo lng('Help') ?>" class="dropdown-item nav-link" href="<?php echo FM_SELF_URL ?>?p=<?php echo urlencode(FM_PATH) ?>&amp;help=2"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> <?php echo lng('Help') ?></a>
...
?>
    ...
    <body class="<?php echo (FM_THEME == "dark") ? 'theme-dark' : ''; ?> <?php echo $isStickyNavBar; ?>">
        ...
        <div class="modal fade" id="uploadModal" tabindex="-1" role="dialog" aria-labelledby="uploadModalLabel" aria-hidden="true" data-bs-theme="<?php echo FM_THEME; ?>">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="uploadModalLabel"><i class="fa fa-cloud-upload fa-fw"></i><?php echo lng('Upload') ?></h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <?php print_external('css-dropzone'); ?>
                        <style>
                        .card-tabs-container.hidden {
                        ...
                        Dropzone.autoDiscover = false;
                        // Initialize Dropzone and handle modal functionality
                        document.addEventListener('DOMContentLoaded', function() {
                            // Handle tab switching in upload modal
                            $('#uploadModal').on('shown.bs.modal', function () {
                                // Bersihkan hasil upload sebelumnya
                                let resultsArea = document.getElementById('upload-results');
                                let resultsList = document.getElementById('upload-results-list');
                                if (resultsArea && resultsList) {
                                    resultsArea.style.display = 'none';
                                    resultsList.innerHTML = '';
                                }
                                // Initialize Dropzone manually untuk modal jika belum ada
                                if (!window.modalDropzone && document.getElementById('fileUploader')) {
                                    window.modalDropzone = new Dropzone("#fileUploader", {
                                        chunking: true,
                                        chunkSize: <?php echo defined('UPLOAD_CHUNK_SIZE') ? UPLOAD_CHUNK_SIZE : 1048576; ?>,
                                        forceChunking: true,
                                        retryChunks: true,
                                        retryChunksLimit: 3,
                                        parallelUploads: 1,
                                        ...
                                        timeout: 120000,
                                        maxFilesize: "<?php echo defined('MAX_UPLOAD_SIZE') ? MAX_UPLOAD_SIZE : 256; ?>",
                                        acceptedFiles: "<?php echo (defined('FM_UPLOAD_EXTENSION') && FM_UPLOAD_EXTENSION) ? '.' . str_replace(',', ',.', FM_UPLOAD_EXTENSION) : ''; ?>",
                                        init: function() {
                                            this.on("sending", function(file, xhr, formData) {
                                                ...
                                                try {
                                                    ...
                                                    if (_response.status == "error") {
                                                        ...
                                                        if (resultsArea && resultsList) {
                                                            ...
                                                            resultsList.innerHTML += '<div class="alert alert-danger mb-2">❌ Error: ' + _response.info + '</div>';
                                                        }
                                                    } else if (_response.status == "success" || _response.info == "file upload successful") {
                                                        toast("File uploaded successfully: " + res.name, 'success');
                                                        let resultsArea = document.getElementById('upload-results');
                                                        let resultsList = document.getElementById('upload-results-list');
                                                        if (resultsArea && resultsList) {
                                                            resultsArea.style.display = 'block';
                                                            resultsList.innerHTML += '<div class="alert alert-success mb-2">✅ Upload Success: ' + res.name + '</div>';
                                                        }
                                                        // Tidak perlu reload di sini, akan ditangani oleh event queuecomplete
                                                    }
                                                } catch (e) {
                                                    ...
                                                    if (resultsArea && resultsList) {
                                                        resultsArea.style.display = 'block';
                                                        resultsList.innerHTML += '<div class="alert alert-warning mb-2">⚠️ Error: Invalid JSON response</div>';
                                                    }
                                                }
                                                ...
                                                if (resultsArea && resultsList) {
                                                    resultsArea.style.display = 'block';
                                                    resultsList.innerHTML += '<div class="alert alert-danger mb-2">❌ Error uploading "' + file.name + '": ' + response + '</div>';
                                                }
                                            }).on("queuecomplete", function() {
                                                // Reload halaman setelah semua file selesai diupload
                                                toast("All uploads completed, reloading page...", 'success');
                                                setTimeout(function() {
                                                    location.reload();
                                                }, 2000);
                                            });
                                ...
                                $('#uploadModal .nav-link').on('click', function(e) {
                                    e.preventDefault();
                                    const target = $(this).attr('href');
                                    // Remove active class from all tabs and contents
                                    $('#uploadModal .nav-link').removeClass('active');
                                    $('#uploadModal .card-tabs-container').addClass('hidden');
        ...
        <div class="modal fade" id="settingsModal" tabindex="-1" role="dialog" aria-labelledby="settingsModalLabel" aria-hidden="true" data-bs-theme="<?php echo FM_THEME; ?>">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="settingsModalLabel"><i class="fa fa-cog fa-fw"></i><?php echo lng('Settings') ?></h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="js-settings-form" action="" method="post" data-type="ajax" onsubmit="return save_settings(this)">
                            ...
                            <div class="form-group row mb-3">
                                ...
                                <div class="col-sm-5">
                                    <select class="form-select" id="js-language" name="js-language">
                                        ...
                                        if (isset($lang_list)) {
                                            foreach ($lang_list as $k => $v) {
                                                echo "<option value='$k' " . getSelectedModal($k) . ">$v</option>";
                                            }
                                        }
                                        ?>
                                    </select>
                            ...
                            <div class="mt-3 mb-3 row">
                                ...
                                <div class="col-sm-9">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" role="switch" id="js-error-report" name="js-error-report" value="true" <?php echo (isset($report_errors) && $report_errors) ? 'checked' : ''; ?> />
                                    </div>
                                </div>
                            </div>
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-9">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" role="switch" id="js-show-hidden" name="js-show-hidden" value="true" <?php echo (isset($show_hidden_files) && $show_hidden_files) ? 'checked' : ''; ?> />
                                    </div>
                                </div>
                            </div>
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-9">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" role="switch" id="js-hide-cols" name="js-hide-cols" value="true" <?php echo (isset($hide_Cols) && $hide_Cols) ? 'checked' : ''; ?> />
                                    </div>
                                </div>
                            </div>
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-5">
                                    <select class="form-select w-100 text-capitalize" id="js-3-0" name="js-theme-3">
                                        <option value='light' <?php if (isset($theme) && $theme == "light") {
                                                                    echo "selected";
                                                                } ?>>
                                            <?php echo lng('light') ?>
                                        </option>
                                        <option value='dark' <?php if (isset($theme) && $theme == "dark") {
                                                                echo "selected";
                                                            } ?>>
                                            <?php echo lng('dark') ?>
                                        </option>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success" onclick="document.getElementById('js-settings-form').submit();"><i class="fa fa-check-circle"></i> <?php echo lng('Save') ?></button>
                    </div>
                </div>
            </div>
        </div>
