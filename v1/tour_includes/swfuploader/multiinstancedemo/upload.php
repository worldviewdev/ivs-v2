...
// Settings
	$save_path = SITE_FS_PATH. "/uploaded_files/uploads/";
...
// Validate file name (for our purposes we'll just remove invalid characters)
	...
	$file_name=$_REQUEST['timestamp']."-".$file_name;
...
// Process the file
	...
	*/
	if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$file_name)) {
		...
		$image_name = md5(uniqid(rand(), true)).'.'.file_ext($file_name);
		rename($save_path.$file_name, UP_FILES_FS_PATH.'/tour_images/'.$image_name);
		db_query("insert into tbl_tour_images set timestamp='".$_REQUEST['timestamp']."', file_name = '$image_name',image_status='Active',recv_date=now(),modified_date=now()");
	}
	exit(0);
